...
/// Behavioral regression tests for Bug 7c4e1a — cold-start retry control flow.
///
...
void main() {
  group('Bug 7c4e1a — cold-start retry behavioral', () {
    ...
    test('exhausting all retries rethrows the last 502', () async {
      // Every attempt fails with 502. With backoffsMs.length == 2, that's
      // 1 initial + 2 retries = 3 total invocations, then rethrow.
      var attempts = 0;
      Future<FunctionResponse> mockInvoker() async {
        attempts++;
    ...
    test('default backoffsMs length matches retry budget (2 retries)', () async {
      // Pins the default schedule's retry count: the helper without a
      // backoffsMs override must allow exactly 2 retries. If someone
      // bumps _coldStartBackoffsMs to 3 or shrinks to 1 without updating
      // this test, the runtime count changes silently — caught here.
      var attempts = 0;
      Future<FunctionResponse> mockInvoker() async {
        attempts++;
        throw const FunctionException(status: 502, details: 'cold');
      ...
      expect(attempts, 3,
          reason: 'Default schedule = 2 retries → 3 total invocations');
    }, timeout: const Timeout(Duration(seconds: 15))); // covers real 1500+4000 ms waits
  });
}
