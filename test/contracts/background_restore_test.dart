...
//     low-risk).
//   * background-restore (flag `bg_restore_enabled`, default OFF): a returning
//     user reaches /home immediately while the cloud restore finishes in the
//     background; the ownership gate stays BLOCKING (cross-account safety,
//     APK #15.4); the in-flight restore is NOT cancelled (single restore, no
//     double-write race); post-restore heals run ref-free, then bump a tick the
//     home screen bridges to invalidateOnRetry.
//
...
// the device walk on the flag rollout (§4.6 — old path preserved verbatim).
...
void main() {
  ...
  group('Obs 4 — bg-restore flag-gated + ownership stays blocking', () {
    test('bg path is gated by bg_restore_enabled (absent key → OFF)', () {
      expect(restoring.contains("configBox.get('bg_restore_enabled')"), isTrue);
    });
