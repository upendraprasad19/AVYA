...
- **Class rule:** Any `.select('col1, col2, ...')` added or refactored MUST have every column live-verified against `information_schema.columns` for the queried table. A field existing *somewhere* in the schema is not proof it exists on THIS table. Cross-check the reader's SoT `fields_read:` against the table's real column set.
