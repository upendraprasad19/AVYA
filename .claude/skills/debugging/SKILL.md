...
- **Prior incidents:** 2026-06-09 diagnose `d3a1c7` (BUG-C) — ai_service `_directHttpCall`/`_directMediaHttpCall` anon-fallback + `sync_service` daily-snapshot/log-client-error direct invokes → AI "down" (obs 3) + the `push_snapshot` 401 that ENABLED the §2.30 plan_json SoT split. Test `test/contracts/edge_function_token_freshness_test.dart`. BUG-G (`a7f2e9`) extended the refresh to the restore entrypoints.
