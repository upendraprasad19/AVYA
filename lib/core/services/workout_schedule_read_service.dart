...
class WorkoutScheduleReadService {
  ...
  /// statuses INVISIBLE to streak + completion-rate math AND to the DISPLAY
  /// read path. CANONICAL home of the predicate:
  ///   - `paused` — the user CHOSE not to train (founder rule: never breaks,
  ///     never burns a freeze).
  ///   - `moved` / `dropped` — TERMINAL rows left by rescheduleWeek in place
  ///     of the old raw delete (writer:
  ///     `tool_dispatcher._executeRescheduleWeek` →
  ///     `WorkoutWriteService.upsertScheduled`). The day's workout lives
  ///     elsewhere now, so the row must read as ABSENT everywhere a
  ///     non-terminal row would render as a live/planned workout —
  ///     `WorkoutRepository.isInvisibleToStreak` (the streak walk, C1)
  ///     DELEGATES here.
  /// Raw-row readers (sync restore, audit) use [getScheduleRowForDate] and
  /// must NOT filter — they need every row.
  ...
  /// RAW row read for [date] — NO invisible-status filtering.
  ///
  /// The only read surface that sees terminal `moved`/`dropped` rows (and
  /// `paused`). For sync restore, audit and streak-walk callers that need
  /// every row. The completed-in-the-future → planned demotion still applies
  /// (that guard is about `completed_at`, not visibility).
  Map<String, dynamic>? getScheduleRowForDate(DateTime date) {
