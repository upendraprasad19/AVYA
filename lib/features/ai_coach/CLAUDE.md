...
| `coach_chat_history_replay` (Unit 2, 2026-07-07) | `CoachInteractionRepository.recentHistoryExchanges` — last N COMPLETE exchanges, oldest→newest, sorted by `created_at`; excludes `kind`-tagged / pending / failed / `mode:'media'` / empty rows + the `coach_memory` singleton — sent as a `history` field via `ai_service.chat` (assembled ONCE in `SendMessageNotifier.send`, threaded into BOTH the primary `:755` and auth-retry `:826` call sites AND both request bodies). Kill-switch `configBox['disable_coach_history']`. **History-poisoning fix (APK +43 obs 2, diagnose `a1c6b9`, 2026-09-16):** a genuine Gemini quota-exhaustion apology ("I had trouble reaching the model...") used to be persisted like real output and replayed into the NEXT turn's history, so the model echoed it as a normal continuation — one transient outage became a self-perpetuating "stuck" chat. `runToolLoop`'s `ToolLoopResult.hadHardFailure` (`tool-loop.ts`, set true only alongside that hardcoded apology) threads through `ai-proxy`'s response body (`had_hard_failure`) → `AiChatResponse.hadHardFailure` → `CoachInteractionRepository.updateInteractionWithResponse(hadHardFailure:)` now writes `entry['failed'] = hadHardFailure` (was unconditionally `false`) — so the pre-existing `failed` filter above now also excludes a hard-failure turn. Also guards the semantic-memory embed so the apology is never embedded either. | `ai-proxy` `capCoachHistory` size-bounds it (§4.4 rule 18: ≤16 entries / ≤2000 chars each / ≤12000 total, oldest-first) → `tool-loop.ts` `repairHistoryAlternation` (shrink-only) seeds `messages[]` BEFORE the current user turn (never 400s Gemini on consecutive same-role turns). Snapshot stays in the system prompt (FC7), so history is a clean `user→model` chain. Complementary to snapshot + `coach_memory` + semantic retrieval. |
...
- `test/contracts/coach_chat_history_replay_writer_to_reader_test.dart` (behavioral — Unit 2: `recentHistoryExchanges` alternating/sorted/filtered; both `chat()` call sites carry history; server-seam source assertions; APK +43 obs 2 — `hadHardFailure:true` excludes the turn from replay, default `false` keeps it) + Deno `supabase/functions/_shared/tool-loop.test.ts` (`repairHistoryAlternation` + `capCoachHistory`) + `supabase/functions/_shared/tool-loop_hard_failure_flag_test.ts` (`hadHardFailure` set true only on genuine retry exhaustion, false on the happy path and the FC2 queued-intent case) + `test/ai_coach/ai_service_had_hard_failure_parse_test.dart` (`had_hard_failure` JSON → `AiChatResponse.hadHardFailure` parse, incl. missing-field default).
