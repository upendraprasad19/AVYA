...
| An already-authenticated user opening a valid `/reset` or `/confirm` link gets silently switched to a different account | **Deliberate, accepted tradeoff, not a gap** — `_authRedirect`'s `isOnReset`/`isOnConfirm` passthroughs return before `isAuthenticated` is even read, so a signed-in user who opens either link (their own stale link, or one forwarded/shared on a device someone else is using) gets `verifyOTP`'d and silently re-authenticated as whichever account the token resolves to, no consent prompt. `_ensureLocalUser`'s cross-account guard is still correctly reused by both flows, so there is no Hive DATA LEAK — only a missing consent signal. The real fix (an interstitial before switching) needs an actual UX decision and applies to both screens equally; tracked as OI-205 rather than fixed ad hoc in whichever screen last touched this code. | OI-205 |
