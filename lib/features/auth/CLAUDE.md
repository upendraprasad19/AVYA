...
- `confirm_email_screen.dart` — **2026-09-16.** Landing screen for the signup
  ...
  `web/.well-known/assetlinks.json`. **Corrected 2026-09-16 (diagnose
  9c4e1a) — the claim that followed here was FALSE and cost a real external
  signup:** anyone without the app does NOT "land on the same route as a
  normal web page" for free. The app uses Flutter's default HashUrlStrategy
  (no `setUrlStrategy` anywhere — same as `/admin`, `lib/features/admin/
  CLAUDE.md:114`), so GoRouter only ever reads the `/#/...` fragment form. A
  bare `/confirm?token_hash=...` needs its own `vercel.json` redirect to
  `/#/confirm`, exactly like `/admin` already has — "one Flutter codebase"
  does NOT mean "no separate implementation" on web; it means the SAME
  redirect precedent must be copied for every new bare-path route. See the
  pitfall row below.
...
| `muster_to_profile_bridge` | **Corrected 2026-09-19 (diagnose d6f1b8) — narrowed from a 3-question bridge to one.** `InductionService.recordMusterAnswer` bridges only `body_part_priorities` to `profile['physique_focus']`; the other 5 muster-style keys (`known_injuries`/`typical_wake_time`/`preferred_workout_time`/`why_now`/`definition_of_winning`) now throw `ArgumentError` — injuries and wake/workout-time are onboarding's/Edit Profile's job only, closing a live clobber bug (muster's answer used to silently overwrite Details' injuries answer on every run). | profile screen reads via `userProfileProvider`; `ai_snapshot_builder.dart` reads the other 3 fields straight off the profile now (see `lib/features/ai_coach/CLAUDE.md`), not a muster-only mirror. |
...
| A deep-link screen's `initState`-only guard silently drops a SECOND, different deep link delivered to the same mounted State | go_router's `pageKey` is derived from the matched PATH ONLY (`ValueKey('/confirm')`, no query string — `go_router` `match.dart`'s `pageKey: ValueKey<String>(newMatchedPath)`), so two navigations to the same path with different query params (e.g. `/confirm?token_hash=A` then `/confirm?token_hash=B`) are the SAME Page to the Navigator, which reuses the Element/State and calls `didUpdateWidget`, NOT `initState`. Combined with Android's `singleTop` launch mode (delivers a second `VIEW` intent to the running Activity via `onNewIntent` without tearing down the current page — e.g. two confirmation-link taps in quick succession), an `initState`-only guard keyed on a bare bool permanently blocks the second link's verification. Fix: key the guard on the VALUE that identifies the request (`_startedFor == tokenHash`, not `_started`), and re-check it in a `didUpdateWidget` override too, not just `initState`. `confirm_email_screen.dart`'s `_maybeStartVerification` is the reference pattern. Caught by B-pass (`docs/reviews/email-confirm-ux-bpass.md` Finding 1), not the original implementation or its own tests — the regression test only ever rebuilt with the SAME token twice. | `docs/reviews/email-confirm-ux-bpass.md` Finding 1 |
| An already-authenticated user opening a valid `/reset` link gets silently switched to a different account (`/confirm` now partially guarded — see body) | **Deliberate, accepted tradeoff, not a gap — but the two routes are NOT equally exposed in practice (corrected 2026-09-16, plan-review round 1).** `_authRedirect`'s `isOnReset`/`isOnConfirm` passthroughs share the identical router-guard shape (both return before `isAuthenticated` is even read), but `/reset` has NO Android App Link registered anywhere — password reset moved to an in-app 6-digit code in 2026-08 (diagnose c9e2b7), so a `/reset` link, when one exists at all, only ever opens in a browser, unable to touch the native app's live session. `/confirm` is `autoVerify="true"`: tapping the link from ANY app hands control directly to the already-running Activity (`onNewIntent`/`singleTop`), no precondition required. So `/confirm`'s silent-switch exposure is materially broader and more automatic, not merely "the same shape extended to a second entry point." `_ensureLocalUser`'s cross-account guard is still correctly reused by both flows, so there is no Hive DATA LEAK — only a missing consent signal. **Interim guard shipped for `/confirm`**: `confirmEmail` now refuses outright (never calls `verifyOTP`) via `AuthNotifier.confirmEmailAuthGuardState` when already authenticated — no more silent switch, but also no same-vs-different-account distinction yet (a harmless self-reconfirm now also needs a sign-out). `/reset` is unchanged. The real fix (an interstitial distinguishing same-account from different-account) needs an actual UX decision and applies to both screens; tracked as OI-205 with the corrected risk picture, rather than fixed ad hoc. | OI-205 |
| A new screen's direct `AuthNotifier.signOut()` call site is missing the OI-51 device-identity-release guard | Any DIRECT (not routed through a shared button widget) call to `ref.read(authNotifierProvider.notifier).signOut()` needs its own `try { await signOut(); } catch (e) { ...; await releaseDeviceSessionIdentity(); }` wrapper, matching `settings_screen.dart`'s `_SignOutButton` — a bare `await signOut()` leaves the device's OneSignal/Crashlytics identity bound if anything throws before the notifier's own internal release runs. Enforced by a DERIVED, tree-wide test (`test/contracts/signout_unbinds_sdk_identity_test.dart`) that walks `lib/` for the call shape, not a hand-maintained list — it will catch a new screen automatically, but only at `flutter test`, i.e. at push, not at a targeted-test-only dev loop. 3rd instance of this exact gap in new code (`confirm_email_screen.dart`, diagnose `d4a8f6`). **Residual, still open:** even with the guard, a genuine `_teardown()` TIMEOUT (as opposed to a throw) leaves `signOut()` returning normally to every caller, so none of the three existing guards can react to it — see debugging skill bug-class 2.65. Tracked as OI-208, not yet fixed. | `docs/diagnoses/2026-09-16-confirm-email-signout-cta-missed-device-identity-release-d4a8f6.md` + debugging skill bug-class 2.65 |
| A non-error `AuthState2` message (e.g. "check your email to confirm") renders in the same red SnackBar as a real sign-in failure | `AuthStatus` had only ONE non-idle/loading/success bucket (`error`), reused for every message regardless of severity. Use `AuthStatus.info` for an expected happy-path next-step message, and route `sign_in_screen.dart`'s renderer through `authToastStyleFor(status)` rather than a hardcoded `AppColors.bad` — it already maps `info` to Wardroom gold (matching `SyncBanner`'s tone for the same class of message). A genuine conflict/failure (e.g. "account already exists") stays `error`/red; the distinction is "does the user need to change what they're doing" vs "signup succeeded, one more step remains." | `test/contracts/auth_toast_info_status_test.dart` |
| A brand-new bare-path GoRoute (`/confirm`, `/foo`, …) 404s/dead-ends on the WEB build for anyone without the app installed, even though the route itself is registered correctly | The app has NO `setUrlStrategy` call anywhere — Flutter web defaults to **HashUrlStrategy**, so GoRouter only ever resolves the `/#/<path>` fragment form. A bare path like `/confirm?token_hash=X` only reaches Vercel's SPA catch-all `rewrites` rule, which serves `index.html` with the URL bar UNCHANGED — `location.hash` stays empty, GoRouter falls back to `initialLocation`, and any query param on the bare path (`token_hash`, here) lives in `location.search`, which the hash strategy never reads. `/admin` already has the fix (`vercel.json` `redirects: /admin → /#/admin`, `lib/features/admin/CLAUDE.md:114`) but `/confirm` shipped in the SAME email-confirm-ux batch WITHOUT the equivalent redirect — confirmed live: a founder's friend's genuine signup attempt hit exactly this, on a phone with no app installed (the common case pre-Play-Store, since Android App Links has nothing to `autoVerify` against otherwise). **Any new bare-path route MUST add its own `vercel.json` redirect in the SAME commit that adds the GoRoute** — the route being registered in `app_router.dart` proves nothing about the web build reaching it at all. | `test/contracts/confirm_web_redirect_test.dart` |
| A user whose confirmation email is lost/expired/never-arrived has no in-app recovery path — only phone OTP had "Resend" | `sign_in_screen.dart`'s email sign-in error handling reused the generic transient-SnackBar path for EVERY failure, including "Email not confirmed" — a self-service-recoverable case distinct from a genuine credential error. Fixed via `AuthNotifier.resendConfirmationEmail` (`auth.resend(type: OtpType.signup)`) + `AuthNotifier.isEmailNotConfirmedMessage` (a case-insensitive match on Supabase's own GoTrue string, deliberately NOT `@visibleForTesting` for the same reason as `alreadyAuthenticatedConfirmMessage` above) gating an inline "Resend it" link on the sign-in step. **Root-caused 2026-09-23 (OI-244), TWO causes, both fixed in code, NOT yet deployed.** (1) `vercel.json`'s `/confirm` redirect forwarded `token_hash` before the `#`, invisible to HashUrlStrategy on every request — fixed via `ConfirmLinkDetector` (diagnose f92d17). (2) `AuthNotifier.confirmEmail` never called `ensureSupabaseReady()` before touching Supabase, unlike its siblings; `/confirm` is a cold, top-level route bypassing `/splash` (the only place `Supabase.initialize()` runs), so the auto-fired call routinely raced an uninitialized `Supabase.instance` — release builds strip the not-ready `assert()`, so the backing `late SupabaseClient client` field threw `LateInitializationError`, landing in the generic catch-all and showing "invalid or has expired" before any network request was attempted. Live-verified via `auth_logs`: zero `/verify` requests reached Supabase across two reproductions. Fixed via the same `ensureSupabaseReady()` guard `signInWithEmail` already used (diagnose 42a98d). Once both deploy, the web fallback path should work end-to-end. **Still genuinely open, unaffected by either fix:** Android App Links not claiming the confirm URL (falls back to a browser instead of the app) — needs the founder's own Play Console → App signing key certificate check against `assetlinks.json`. | `docs/diagnoses/2026-09-23-email-not-confirmed-no-resend-path-f6c2a9.md`, `docs/diagnoses/2026-09-23-confirm-link-token-hash-lost-in-vercel-redirect-f92d17.md`, `docs/diagnoses/2026-09-23-confirm-email-supabase-not-ready-race-42a98d.md` |
...
- `test/auth/confirm_email_screen_test.dart` — missing/empty `token_hash` never reaches `confirmEmail`; a present one calls it exactly once and doesn't re-fire on the same token; a genuinely DIFFERENT token delivered to the same mounted State (`didUpdateWidget`) re-triggers; success navigates to `/restoring` (real `GoRouter` harness); the already-signed-in guard state shows a distinct SIGN OUT CTA (never the generic GO TO SIGN IN button) that actually calls `signOut` before navigating to `/sign-in` (round 2 Finding 1 — a bare `context.go` there is a silent no-op per `_authRedirect`). All production-code guards mutation-proven.
- `test/contracts/confirm_email_error_mapping_test.dart` — pure-function tests for `confirmEmailErrorState` (Timeout/StateError/AuthException/generic → correct message) and `confirmEmailAuthGuardState` (OI-205 interim guard: blocks when already authenticated, does NOT block the unauthenticated common case). Mutation-proven; no live Supabase call needed since both are extracted pure functions.
- `test/contracts/confirm_email_readiness_behavioral_test.dart` — `confirmEmail`'s `ensureSupabaseReady()` guard (diagnose 42a98d, OI-244): an overridden not-ready fake short-circuits before `verifyOTP` is ever reached; the REAL (non-overridden) helper, which fails on this VM test process's empty `.env`, surfaces its own accurate message rather than the generic "invalid or expired" fallback. Mutation-proven — the mutated run reproduced the exact live production error string.
- `test/contracts/confirm_web_redirect_test.dart` — pins `vercel.json`'s `/confirm` + `/confirm/` redirects (parsed as real JSON) to `/#/confirm`, that the `/admin` redirect and the `.well-known/` rewrite exclusion both survive, and that `app_router.dart`'s `/confirm` route still reads `token_hash` off the query string. Presence/shape only — no `flutter test` can exercise real Vercel routing; live verification is a deploy + a real GET (tier 11).
- `test/contracts/auth_toast_info_status_test.dart` — behavioral coverage of `authToastStyleFor` (info → Wardroom gold, error → red, unchanged); structural pins that `signUpWithEmail`'s confirmation-pending message uses `AuthStatus.info` while "account already exists" stays `AuthStatus.error`, and that the SnackBar renderer goes through the shared function rather than a hardcoded color.
- `test/contracts/is_email_not_confirmed_message_test.dart` — pure-function coverage of `isEmailNotConfirmedMessage` (case-insensitive match, mirror cases: a different auth failure and the sign-up confirmation-pending info message must NOT match).
- `test/auth/sign_in_screen_resend_confirmation_test.dart` — the resend-confirmation affordance shows only on "Email not confirmed" (not on a wrong-password failure), tapping it calls `resendConfirmationEmail` with the trimmed entered email exactly once, and "CHANGE EMAIL" clears a previously-shown affordance. Mutation-proven (diagnose f6c2a9).
