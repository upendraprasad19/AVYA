...
| A user whose confirmation email is lost/expired/never-arrived has no in-app recovery path — only phone OTP had "Resend" | `sign_in_screen.dart`'s email sign-in error handling reused the generic transient-SnackBar path for EVERY failure, including "Email not confirmed" — a self-service-recoverable case distinct from a genuine credential error. Fixed via `AuthNotifier.resendConfirmationEmail` (`auth.resend(type: OtpType.signup)`) + `AuthNotifier.isEmailNotConfirmedMessage` (a case-insensitive match on Supabase's own GoTrue string, deliberately NOT `@visibleForTesting` for the same reason as `alreadyAuthenticatedConfirmMessage` above) gating an inline "Resend it" link on the sign-in step. **Residual, still open (2026-09-23, diagnose f6c2a9):** a live audit found 6 of 9 real external signups in the prior 10 days never completed confirmation at all, and the one founder-forwarded report (`sumitk142003@gmail.com`) showed `auth.one_time_tokens` still holding the ORIGINAL unconsumed token — i.e. whatever the founder observed looking like a "confirmed" state, `verifyOTP` had not actually succeeded server-side. `auth.audit_log_entries` is empty project-wide, so there is no server-side trail to distinguish "App Link failed to auto-verify and opened a browser that never completed the call" from "the founder glimpsed the loading state and misread it" from some other cause. Custom SMTP (Brevo) is already configured, ruling out this table's own "no custom SMTP" row as the explanation. NOT yet root-caused, tracked as OI-244. | `docs/diagnoses/2026-09-23-email-not-confirmed-no-resend-path-f6c2a9.md` |
