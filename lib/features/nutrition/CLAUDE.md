...
- `log_food_sheet.dart` — the single bottom sheet for logging food (5 tabs: AI / Scan / Cart / Barcode / Search), replacing the retired `log_to_slot_sheet.dart` (deleted 2026-09-20, obs 5). `showLogFoodSheet(context, {initial, lockedSlot})`; when `lockedSlot` is set it locks `mealTypeProvider` to that slot for the sheet's lifetime via a deferred `addPostFrameCallback` and retitles to "LOG TO {SLOT}".
