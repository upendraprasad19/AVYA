...
| `nutrition_log_retag` (obs 1, 2026-09-20) | `nutrition_write_service.dart` `moveMealLog({logKey, newMealType, macroUpdates})` — rekeys the `nlog_*` row (meal_type is embedded in the key) rather than a field-only update, clamping totals AFTER any collision-merge (diagnose `a3f6c9` — clamping before merge let merged totals bypass the FC6 absurd-value clamp). LOCAL-ONLY cloud consistency, same accepted residual as `deleteLog`/`editLog`. | Edit Macros sheet's meal-slot selector (`nutrition_screen.dart`); `TodaysMealsCard` re-buckets by the new `meal_type`. |
...
| Diet-plan meals not visible on nutrition screen | `TodaysMealsCard` renders a "SUGGESTED" chip hint on empty slots via `dietPlanProvider` (reads `configBox['saved_diet_plan']`). If hints don't show after a user saves a plan, confirm `diet_plan_screen._savePlan` still calls `ref.invalidate(dietPlanProvider)` after `saveDietPlan`. ⚠ Corrected 2026-09-20 (diagnose e1c5b8) — the old `showFoodSearchSheet(initialQuery:...)` tap-to-log pre-fill no longer exists (that file was deleted, dead code); tapping the empty-slot CTA today opens `showLogFoodSheet(context, lockedSlot: slot)` with NO food-name pre-fill. | (relocated 2026-05-18 — see docs/diagnoses/INDEX.md) |
...
| Meal-slot time-of-day inference drifts across call sites | The current meal slot has exactly ONE source of truth: `mealTypeProvider` (`nutrition_provider.dart`). Before 2026-09-20, `search_mode_body.dart` and `barcode_scan_sheet.dart` each carried their own inline `_mealTypeForNow()`-style time-window inference, independently of `mealTypeProvider` and of each other — so opening "LOG TO LUNCH" → Search or Barcode tab could silently log to whatever time-of-day inferred instead of the tapped slot. Both now read `ref.watch`/`ref.read(mealTypeProvider)` exclusively; never reintroduce a local time-of-day inference in any nutrition logging surface. | diagnose `2026-09-20-meal-slot-inference-drift-1261a4` |
