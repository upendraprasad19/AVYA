...
| `TELEGRAM_BOT_TOKEN` | `morning-alert` (per-user Telegram delivery), `founder-digest` (the founder's daily digest, OI-153), `telegram-admin-bot` (webhook replies to every command, telegram-admin-bot batch 2026-09-14), `alert-critical-notify` (pushes a critical alert immediately on dispatch, same batch). ⚠ A Deno fetch error's `.message` embeds the request URL, which carries this token — `founder-digest` logs `err.name` only; never log or `String()` a Telegram fetch error. | 2026-09-14 — updated for the telegram-admin-bot batch's two new consumers (row added retroactively; see R2-13). |
| `FOUNDER_TELEGRAM_CHAT_ID` | `founder-digest` (the chat the digest is sent to), `telegram-admin-bot` (the single allowlisted chat id `isAuthorizedTelegramSender` checks every webhook against), `alert-critical-notify` (the chat a critical alert is pushed to). Not a credential on its own, but paired with the token it addresses the founder, so it stays a secret rather than a constant. | 2026-09-14 — updated for the telegram-admin-bot batch's two new consumers (row added retroactively; see R2-13). |
...
| `TELEGRAM_WEBHOOK_SECRET` | `telegram-admin-bot` only — the `secret_token` Telegram echoes back on every webhook POST (registered via `setWebhook`'s `secret_token` param). Checked in-handler against `X-Telegram-Bot-Api-Secret-Token`, alongside a single allowlisted chat id (`FOUNDER_TELEGRAM_CHAT_ID`) — both must match; neither alone is sufficient (`isAuthorizedTelegramSender`). Generated 2026-09-14 via `openssl rand -hex 32`, same standard as `CRON_SECRET`. | 2026-09-14 — webhook registered, `/status` smoke test confirmed live. |
