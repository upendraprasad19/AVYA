...
| 110 / 144 | `alert_cron_function_dead` (32) | `47 6 * * *` | 12:17 | (intra-DB) | n/a | Per-function complement: a function that succeeded within 14d but not in 8d is presumed dead. Catches the shape `alert_cron_silence` misses. Migration 144 excludes the trigger-dispatched `alert-critical-notify` (silence is healthy for it; a critical about it would dispatch it and reset its own clock — OI-179 R2-11). ⚠ The `>= 8` vs 7-day-prune defect (OI-179) is unchanged |
| 141 / 144 | `db_maintenance_nightly` (41) | `30 3 * * *` | 09:00 | (intra-DB) | n/a | Folded 6 jobs by migration 141 (2026-09-22 disk-IO audit). ⚠ **141's form FAILED EVERY RUN** 09-22 → 09-26 (`VACUUM cannot run inside a transaction block`): pg_cron runs a multi-statement command as ONE transaction, so the two VACUUMs failed AND rolled back the four cleanups with them — no retention ran for 5 nights (diagnose d6b2f9, OI-247). **Migration 144** (`cron.alter_job`, same jobid 41) reduced it to the four cleanups: `cleanup_cron_call_log`, `cleanup_usage_counters`, `cleanup_cron_job_run_details`, `cleanup_client_errors`. The VACUUMs are back in their own single-statement jobs (rows below). Pinned by `test/contracts/cron_vacuum_single_statement_test.dart`, which scans EVERY migration for a VACUUM beside another statement. `clean_orphan_media_daily` deliberately NOT folded in — it's an HTTP call, not pure SQL |
| 144 | `jrd_vacuum_daily` (44) | `40 3 * * *` | 09:10 | (intra-DB) | n/a | `VACUUM (ANALYZE) cron.job_run_details` — single statement, 10 min after `db_maintenance_nightly`'s cleanups. Same single-statement command as the pre-141 job (121, jobid 35), which succeeded nightly 09-06 → 09-20 — NEW jobid and a new time slot (was 04:38), so 35's run history does not carry over |
| 144 | `client_errors_vacuum_daily` (45) | `43 3 * * *` | 09:13 | (intra-DB) | n/a | `VACUUM (ANALYZE) public.client_errors` — single statement; same restoration as `jrd_vacuum_daily` (pre-141 jobid 36, was 04:41; new jobid) |
...
| 121, unscheduled 141, RESTORED 144 | `jrd_vacuum_daily` | Folded into `db_maintenance_nightly` by 141 — which made that job fail every night (VACUUM in a transaction block). **Re-scheduled as its own job by migration 144**; see the active row above. Kept here as history |
| 121, unscheduled 141, RESTORED 144 | `client_errors_vacuum_daily` | Folded into `db_maintenance_nightly` by 141 — which made that job fail every night (VACUUM in a transaction block). **Re-scheduled as its own job by migration 144**; see the active row above. Kept here as history |
