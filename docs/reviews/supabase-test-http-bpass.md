...
- **why it matters:** it does not invalidate the raise (every observed failure was a timeout, none an
  ...
  nothing.
- **fix applied:** filed as **OI-120** (deliberately separate from OI-122, which is scoped to the local
...
## Finding 4 — P2 — unbounded_local_walk (quantifies OI-122)
...
- **fix applied:** the multiplier added to OI-122's evidence. No separate action — OI-122 already owns
...
- **secrets_in_tree** — `QA_Test_2024!` / `qa@icanbefitter.com` are **pre-existing**: `git blame` puts
  both at commit `de94c9f38`, 2026-04-04, four months before this branch. Not introduced here. OI-121
