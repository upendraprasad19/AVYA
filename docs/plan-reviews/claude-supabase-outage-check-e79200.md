...
> This branch's canonical plan-review filename (`recordSlug()`: full branch name, `/`→`-`) was
> previously used by an EARLIER, already-merged batch on this same reused branch — the
> swap-logged-sets-format fix (bug `a4c7d1`, commit `5469e2f3`, merged before this session). That
> record served its purpose at that earlier merge and remains in git history at `5469e2f3`; this
> file now covers the CURRENT batch being merged. A prior draft of this record was mistakenly
> authored under `docs/plan-reviews/supabase-outage-check-e79200.md` (missing the `claude-`
> branch-namespace prefix `recordSlug()` requires), which the keystone gate never reads — caught
> when `check_plan_review_record_exists.dart` failed on the actual merge commit, reading the
> stale swap-logged-sets record instead. See `feedback_closes_oi_trailer_can_go_stale_from_concurrent_gate`-adjacent lesson in harness memory (a sibling gate-timing class, not the same gate).
