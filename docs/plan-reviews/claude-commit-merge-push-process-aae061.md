...
3. **MAJOR — OI-102 left asserting a false "What's wrong".** Fixed: OI-102 → CLOSED (its trigger
   is gone), with its unanswered measurement question carried forward as a well-formed **OI-106**
   rather than dying with it. `OPEN_INDEX.md` regenerated.
   (Filed as OI-105 on this branch; renumbered to **OI-106** at merge time — an unrelated batch
   landed its own OI-105 on `main` first. `main` wins the number,
   `feedback_main_is_source_of_truth`.)
