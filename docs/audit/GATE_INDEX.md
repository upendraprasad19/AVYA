...
the next free number: **49**. Declare it canonically as `// Gate: N` on its
...
| 18 ⚠ COLLISION | `check_app_version_matches_pubspec.dart` | Build-gate script: asserts `AppConstants.appVersion` in | — |
...
| 44 ⚠ COLLISION | `check_device_tests_exist.dart` | assert that the 4 named Patrol device-CI flow files exist in `integration_test/device/` and | — |
| 25 | `check_diagnose_index_fresh.dart` | assert that `docs/diagnoses/INDEX.md` enumerates every diagnose-doc on disk. | — |
| 18 ⚠ COLLISION | `check_doc_internal_consistency.dart` | known-drift pairs across CLAUDE.md + AGENTS.md must agree. | — |
...
| 19 ⚠ COLLISION | `check_hive_map_field_drift.dart` | Hive Map field-key drift detector (Theme G, closes-diagnose | — |
...
| 44 ⚠ COLLISION | `check_nested_claude_md_content.dart` | Nested CLAUDE.md content quality | — |
| 23 ⚠ COLLISION | `check_nlog_key_canonical.dart` | Drift-fix batch 2026-05-24 / F2 nutrition — source-grep gate. Pins | — |
...
| 18 ⚠ COLLISION | `check_reader_manifest_complete.dart` | Audit gate (build-apk Gate 18) — enforces the reader-side manifest in | — |
...
| 19 ⚠ COLLISION | `check_schema_payload_parity.dart` | every NOT NULL column on user-tagged Supabase tables must | — |
| 23 ⚠ COLLISION | `check_secrets_gitignored.dart` | assert that Android signing artifacts and other sensitive secret patterns are never tracked | — |
...
| 7 ⚠ COLLISION | `check_sot_registry_completeness.dart` | SoT registry completeness. | — |
...
| 7 ⚠ COLLISION | `check_writeservice_only.dart` | All writes to workoutBox / nutritionBox / healthBox must go through the | — |
