...
>
> **History:** lenses L1-L20 are the default-charter set (5-domain audits + 2026-05-12 Master+Codex dual-pass). L21-L25 were surfaced by Hermes audit 2026-05-17 (TDZ/schema-parity/auth-defense-in-depth). L26-L33 were surfaced by external "Hermes" cross-check on 2026-05-11 (`feedback_audit_methodology_lenses.md` R1+R2). L34-L41 were added prophylactically for next audit. L42-L53 were added during the 2026-05-20 tech-debt audit / B1 (each backed by a permanent `scripts/check_*.dart` gate). Memory file is authoritative; this file is the dispatch checklist. **Total: 54 lenses.** (L54 = context-artifact cost, added 2026-08-30 with `scripts/check_context_artifact_budget.dart`.)

---

## How to use

1. Pick the lens set: `--all` (54 lenses) for quarterly comprehensive; `--security` (L2/L21/L23/L29) for pre-launch; `--p0-blockers` (L1/L21/L22/L23) for emergency triage.
...
| **L54** | **Context-artifact cost** | For every artifact loaded into agent context on EVERY session or EVERY lookup — root `CLAUDE.md`, the harness `MEMORY.md`, `docs/audit/OPEN_INDEX.md`, any always-read index — is its size MEASURED, and does every prose claim ABOUT that size still hold? Ask both halves: the bytes, and the sentence describing the bytes. | `scripts/check_context_artifact_budget.dart` + baselines in `backups/context_artifact_sizes.json` (soft 15% warns, hard 50% blocks, fails open) | `test/scripts/context_budget_lib_test.dart` + `test/scripts/context_artifact_budget_e2e_test.dart` | **2026-08-30 — the gap this registry itself had.** `open_issues.md` grew 67,895 → 357,664 B over **31 days** (2026-07-30 → 08-30): ~9,347 B/day, ~2,600 tok/day. ⚠ An earlier draft of this row said "30 days (~+3,200 tok/day)"; that figure was retracted in `context_budget_lib.dart` and SURVIVED HERE for two review rounds, because a number restated in a second document is corrected in neither. Prefer pointing at one source over restating it and 46% of it was CLOSED entries; `OPEN_INDEX.md` reached 5.0× the "~950 tokens" CLAUDE.md §7 advertised — a claim TRUE when written (`e4bc9040`, 2026-07-29, 3,759 B) that nothing re-derived. **Why no audit found it:** every review is diff-scoped (`/code-review` reads `git diff --cached`), each commit added ~3 KB and was individually innocent — the lesson CLAUDE.md:477 already records for `check_no_deferral_euphemism` and never generalised; §4.8 makes an audit declare its lenses, and NO lens named this; the tech-debt Documentation pass asks whether docs are CORRECT, not AFFORDABLE; and the repo's one size gate (`check_apk_size_within_bounds`, Gate 13) guards the artifact USERS download. **The gate covers three named files — this lens is what lets an audit ask the question of an artifact nobody has thought of yet.** |
...
| 2026-09-13 (Hermes E-pass, `oi153-pro-media-caps`, catastrophic by CONTENT) | L1, L14, L21, L22, L23, L31, L35, L40 (targeted 8) | 19 (1 P0 + 2 P1 + 16 P2), 0 false_alarm, 3 partial | L23 found a pre-existing P0 (path traversal through the OI-28 guard, `c7e2a4`) in a function the batch redeployed without touching its guard — run L23 on any service-role redeploy. Report `docs/audit/2026-09-13-hermes-oi153-pro-media-caps.md`. |
