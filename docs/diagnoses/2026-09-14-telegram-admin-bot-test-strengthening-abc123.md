...
1. **HTML-escaping in cmdCron:** Added `escapeHtml(info.status)` wrapper (line 377)
...
**Line 377 (cmdCron):** Added HTML escaping to status field to prevent XSS via Telegram message rendering.
