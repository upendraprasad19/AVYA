...
touched_layers_checked:
  ...
  - { tier: 6, name: edge_function_deploy, status: fixed_in_this_batch, evidence: "ai-media-proxy source fixed in git (67ba6ba4); deno check (local Deno 2.9.6, --node-modules-dir=none) PASS on the whole tree. ⚠ NOT DEPLOYED as of this document's last commit — live is v23 (2026-09-10, the PRE-fix bundle: countProImageAnalysesToday + the 429 path still present, verified by the B-pass via the Management API on 2026-09-13), so the dormant cap and the uncapped PRO video are LIVE until the post-merge deploy. The deploy is the last step of this same batch, pre-authorized by the founder on 2026-09-12; it ships with the CURRENT live verify_jwt, an anon-Bearer boot probe, a real-user-token smoke and the multipart /body byte-identity check (deploy-rollback skill 6.9). The deployed version is recorded in the closure ledger (docs/audit/oi153-pro-media-caps.closure.yaml) and the project memory, NOT here — this file cannot be amended after the deploy without another commit. The first draft of this line claimed the deploy had happened; the B-pass caught it (its Finding 1) against live state." }
  ...
  - { tier: 9, name: storage, status: verified, evidence: "The Storage fetch (fetchImageAsBase64, SSRF allowlist + user-scope assertion) is untouched and now runs BEFORE the consume — a rejected or racing upload never spends a unit (T1 pins the order)." }
