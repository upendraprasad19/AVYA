...
touched_layers_checked:
  ...
  - { tier: 6, name: edge_function_deploy, status: fixed_in_this_batch, evidence: "ai-media-proxy source fixed in git; deno check (local Deno 2.9.6, --node-modules-dir=none) PASS on the edited files. Deployed on the founder's explicit go, byte-identity verified via the multipart /body endpoint (deploy-rollback skill 6.9) — version recorded in the retrospective." }
