...
touched_layers_checked:
  ...
  - { tier: 6_edge_function_code_vs_deploy, status: pending_explicit_authorization, evidence: "15 Edge Functions changed in git and NOT yet deployed to dedsavbjuwgarrhphgnl. Per CLAUDE.md 4.3 a live deploy needs its own explicit authorization beyond plan approval. Until each is deployed the live fleet still truncates — the fix is inert in production. No ordering constraint between the 15 (no migration dependency), so they can deploy in any order; severity order (Class 1 first) is the plan." }
