...
mutation_proven:
  ...
  result: "CORRECTED 2026-09-23 by B-pass Finding 2 (docs/reviews/3aa28693fb6f-review.md) — the original count below was wrong and is kept struck through per CLAUDE.md §4.4 rule 21 rather than silently edited, since the review independently re-derived the true count. Only **1 of 6** tests reddens: the case pinning the ACTUAL live-broken shape. The 'prefers fragment when both present' test's URL (`.../?token_hash=stale_query#/confirm?token_hash=fresh_fragment`) is resolved entirely by the FIRST branch (the fragment segment has its own `?`, so `fromFragment` is non-null and `detect()` returns before ever reaching `uri.queryParameters`) — removing the `fromQuery` fallback cannot affect it, so it stays green throughout. ~~2 of 6 tests reddened: the case pinning the ACTUAL live-broken shape, and the case asserting precedence when both shapes are present~~ — that second claim was never actually true; the precedence test does not exercise the `fromQuery` branch at all. The other 4 (fragment-shape detection, absence, unrelated URL, empty value) correctly stayed green throughout, confirming they exercise the OTHER branch/negative paths, not this one. The underlying protection is still real and correct — this was a documentation-accuracy error in the count, not a coverage gap in the shipped fix."
  confirmed_applied: "Ran `flutter test test/contracts/confirm_link_detector_test.dart` before, during (mutated), and after (reverted) — 6/6 green → 5/6 green (1 failing: 'detects token_hash from the ACTUAL live-broken shape', Expected 'pkce_abc123' Actual <null>) → 6/6 green. Re-verified independently by the B-pass reviewer (docs/reviews/3aa28693fb6f-review.md Finding 2), same result."
residual: |
