...
- `user_profile.preferred_workout_time TEXT` (migration 063, 2026-05-13). Originally captured muster Q4's "preferred workout time" answer; **the muster question was retired 2026-09-19 (diagnose d6f1b8)** — `InductionService.recordMusterAnswer` now rejects `preferred_workout_time` outright (`ArgumentError`), closing that writer path. `"HH:MM"` 24-hour. NULL when not collected. Written ONLY by Edit Profile's picker tile now (`_pickPreferredWorkoutTime`); a pre-restructure user's already-stored muster answer still one-shot-migrates via `backfillMusterToProfileIfNeeded`. Read by AI context (`ai_snapshot_builder._getInductionAndMusterKeys()` now reads this profile column directly rather than a coachBox mirror — see `docs/architecture/sync.md`) and Edit Profile.
