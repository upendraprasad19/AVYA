...
- **`AUTH-02`** — The sign-in surface (`sign_in_screen.dart`) offers Google OAuth and an inline email field on ONE entry screen (2026-08 redesign — previously "ENLIST VIA EMAIL" opened a separate email-entry screen; that step was merged into the main view to cut the funnel from 3 screens to 2). WHEN a user enters an email and taps CONTINUE THEN the app checks registration status server-side (`email_is_registered` RPC) and branches automatically to a password-only sign-in step or a sign-up step (password + optional referral + privacy checkbox) — no manual sign-in/sign-up toggle. **Verify:** `test/contracts/email_registration_gate_state_machine_test.dart`, `test/auth/sign_in_screen_email_gate_test.dart` (widget). **Live-verified 2026-06-21** for the branching logic (pre-redesign screen count; re-verify the merged layout live).
- **`AUTH-03`** — WHEN a user requests forgot-password THEN `forgot_password_sheet.dart` sends a reset link via `Supabase.auth.resetPasswordForEmail` with `redirectTo` set to the prod Web origin (never localhost). **Verify:** `test/contracts/password_reset_redirect_flow_test.dart`.
- **`AUTH-03b`** — WHEN a user submits a valid new password on `/reset` THEN `ResetPasswordScreen` calls `auth.updateUser`, signs out, and navigates explicitly to `/sign-in` — GoRouter has no `refreshListenable` tied to Supabase auth state and `/reset` is deliberately exempt from `_authRedirect`, so nothing else would move the user off the route (diagnose `c8f1d3`, the "stuck screen" bug). **Verify:** `test/contracts/password_reset_redirect_flow_test.dart` (behavioral group).
- **`AUTH-04`** — Phone OTP is NOT wired in prod (Twilio account created but not connected to Supabase Auth) — it fails silently. **Verify:** `(unverified — confirm)` per `project_pending_twilio_setup.md`; needs prod smoke check.
- **`AUTH-04b`** — WHEN a user taps "ENLIST VIA GOOGLE" THEN `AuthNotifier.signInWithGoogle` redirects via `signInWithOAuth`, with `redirectTo` branched on `kIsWeb` (mobile custom scheme vs. prod web origin), and Android catches the return via a manifest deep-link intent-filter. Requires account-side setup (Google Cloud OAuth client + Supabase provider config) outside this repo — see diagnose f2b8a1 for what was previously missing. **Verify:** `test/contracts/google_oauth_redirect_flow_test.dart`; live end-to-end tap-through still pending at time of writing — do not assume this claim without re-checking.
...
- **`AUTH-06`** — WHEN the user accepts Terms at sign-up THEN acceptance is persisted (email signup: `_ensureLocalUser` stamps Hive after `HiveUserSession.openForUser`; Google OAuth / phone OTP: `AuthSessionBootstrapper.hydrateFromCloud` fallback). **Verify:** `test/contracts/terms_acceptance_behavioral_test.dart`, `test/contracts/terms_acceptance_writer_to_reader_test.dart`.
...
- **`ONB-13`** — **Narrowed 2026-09-19 (diagnose d6f1b8).** Only muster's `body_part_priorities` answer bridges into a profile field, via `InductionService.recordMusterAnswer` → `_bridgeToProfile` (`body_part_priorities[0]→physique_focus`, single-element only). `known_injuries`, `typical_wake_time`, and `preferred_workout_time` are now REJECTED by `recordMusterAnswer`'s `ArgumentError` guard — injuries stays onboarding Details' job, wake/workout-time stays Edit Profile's job (both already had their own UI), closing a live clobber bug where muster's answer silently overwrote Details' injuries answer. `ai_snapshot_builder._getInductionAndMusterKeys()` reads all 4 non-bridged-writer fields straight off the profile now, not a muster-only coachBox mirror. **Verify:** `test/contracts/muster_profile_bridge_test.dart` (3 retired keys throw, `body_part_priorities` bridge unchanged), `test/contracts/muster_to_profile_bridge_behavioral_test.dart` (behavioral pin + retired-keys-write-nothing case), `test/ai_coach/snapshot_keys_test.dart` (coachBox-only write no longer reaches the snapshot), `test/contracts/muster_bridge_backfill_test.dart` (pre-restructure backfill unchanged), `test/contracts/muster_question_count_test.dart`.
...
- **`HOME-04A`** — A past scheduled day with `status='paused'` — and the terminal `moved`/`dropped` rows rescheduleWeek leaves — is INVISIBLE to the streak walk-back AND to `completionRateOverWindow`: never breaks, never consumes a freeze, never counts against rank completion-rate gates. Rationale: pausing is a legitimate choice, not a miss. Full rule: `docs/architecture/business-rules.md` § Paused days × streak (spec 2026-09-18, founder decision). **Verify:** `test/contracts/streak_paused_day_not_missed_test.dart`.
- **`HOME-05`** — `WorkoutRepository.currentStreak()` is a pure READ and MUST NOT consume a freeze as a side effect of rank/splash/post-workout evaluation. The mutating half is the explicitly-named `consumeMissedDayIfFreezeAvailable()`. (The merged `calculateCurrentStreak()` shim was DELETED in OI-44 Unit 6, 2026-08-02 — a query-named method that delegated straight to the mutator.) **Verify:** `test/contracts/streak_currentstreak_is_pure_test.dart`.
...
- **`NUT-05`** — WHEN AI text analysis runs THEN `food_logger_section._analyse` calls `food-text-analysis` Edge Function; the daily counter increments at the API-call site (NOT at save). Server caps 10/day free, 200/day PRO (free arm lowered 50→10 in b8f4c2 to match the client). **Verify:** `test/contracts/food_text_analysis_daily_cap_test.dart`.
...
- **`NUT-08`** — Scan meal: client cap 3/day free, 10/day PRO (soft warning at 7/10); server abuse cap 20/day combined with cart_auditor (raised from 15, 2026-07-29); results are fully editable via `_ScanResultEditor` (meal name, per-item name/kcal/P/C/F/Fi, +Add/X Delete) with totals recomputing on every keystroke. **Verify:** needs test (widget); caps via `business-rules.md`.
...
- **`COACH-12`** — Photo upload via `ai-media-proxy` (`verify_jwt: true` + manual JWT + tier flag): a FREE user gets 5 lifetime image reads (`usage_counters` key `free_image_analysis`, OI-162 slice 3b) then the paywall reply; VIDEO is PRO-only (⚠ this assertion read "Photo/video upload is PRO-only" until 2026-09-12 — stale since slice 3b); it is SSRF-allowlisted to `${SUPABASE_URL}/storage/v1/object/` (`progress-photos` + `chat-attachments` buckets only); image max 5MB. **Verify:** `test/contracts/chat_media_signed_url_test.dart`, `test/contracts/ai_media_proxy_status_code_classification_test.dart`, `test/contracts/ai_media_proxy_telemetry_test.dart`, `test/contracts/edge_function_safety_test.dart`.
...
- **`COACH-20`** — PRO media reads are capped per IST day — **50 images, 10 videos** (OI-153, 2026-09-12) — by ONE atomic `consume_quota` call in `ai-media-proxy` placed AFTER the Storage fetch (a rejected upload never spends a unit) and BEFORE the Gemini call (the spend is bounded under concurrency); the key AND the cap are both selected by `isVideo`. A reached cap returns HTTP 200 `gated: true` with a rank-free Bridge reply naming the midnight-IST reset (`gate_reason: pro_image_daily_limit_reached` / `pro_video_daily_limit_reached`, `resets_at`), rendered by every installed APK as an ordinary coach bubble — never a 429, never the paywall. A ledger error (`pro_quota_unavailable`) or a `subscriptions` read error (`tier_unavailable`) refuses honestly for either tier. **Verify:** `test/contracts/pro_media_daily_caps_writer_to_reader_test.dart`, `test/contracts/coach_replies_test.dart` (copy mirror, no "unlimited"), `test/sql/oi153_pro_media_caps_live_verify.sql` (live, BEGIN…ROLLBACK).
...
- **Auth/Onboarding:** `AUTH-03`, `AUTH-04` (unverified — Twilio prod), `AUTH-07` (widget), `AUTH-12`, `AUTH-13` (assert `since` constant), `AUTH-18`, `ONB-01`, `ONB-02`, `ONB-03`, `ONB-04`, `ONB-06`, `ONB-07`, `ONB-10`, `ONB-11` (scheduling), `ONB-14`. (`AUTH-02` now covered — see its own Verify line.)
...
- **Coach:** `COACH-08` (tier count + intent TTL), `COACH-09` (retrieval fallback), `COACH-11` (cron), `COACH-14` (error mapping), `COACH-15`, `COACH-16`, `COACH-17`, `COACH-19` (cron dedup), `COACH-20` (PRO media daily caps — contract + live SQL, not a behavioural client test).
