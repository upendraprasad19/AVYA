...
- **`AUTH-02`** — The sign-in surface (`sign_in_screen.dart`) offers Email and Google OAuth entry (Phone OTP entry point hidden behind `_kEnablePhoneEnlist = false` pending Twilio wiring — see `AUTH-04`; the phone code path itself is preserved, not deleted). WHEN a user chooses Email THEN they enter only an email address first; the app checks registration status server-side (`email_is_registered` RPC) and branches automatically to a password-only sign-in step or a sign-up step (password + optional referral + privacy checkbox) — no manual sign-in/sign-up toggle. **Verify:** `test/contracts/email_registration_gate_state_machine_test.dart`, `test/auth/sign_in_screen_email_gate_test.dart` (widget). **Live-verified 2026-06-21** (superseded 2026-07 by the email-first flow + phone hide — re-verify live).
