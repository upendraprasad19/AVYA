...
- **Nutrition F4 P2** + **Workout F4 P2** — migration 068 ships `nutrition_log_items.fiber NUMERIC DEFAULT 0` (additive) + atomic rename `workout_logs.exercise_name` → `workout_name` (column was always a session label e.g. "Push A", never per-exercise; founder choice — sole tester, no dual-write phase). UNIQUE INDEX dropped + renamed + recreated; weekly-report Edge Function (sole `workout_logs.exercise_name` reader) redeployed v21. Source-tree file `068_drift_fix_batch.sql` lives alongside the existing `068_cron_call_log.sql` (prefix collision; `applied_migrations.json` records `"068b_drift_fix_batch"`).
