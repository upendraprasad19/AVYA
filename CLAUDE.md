...
| OneSignal player_id never written by client | Migration 049 added `user_progress.onesignal_player_id` for the `delete-account` unsub flow, but the Flutter app does NOT yet write this column. `delete-account` reads the column, finds NULL, gracefully skips OneSignal unsub (logs warning, continues). Result: deleted accounts may continue to receive push notifications until the OS uninstalls the app. **Fix-forward (deferred to Test #12):** in `auth_provider.dart` after `OneSignal.login(user.id)`, fetch `OneSignal.User.pushSubscription.id` and upsert it to `user_progress.onesignal_player_id`. |
