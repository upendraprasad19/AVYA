...
- **Commits and pushes go through `scripts/safe_commit.sh` / `scripts/safe_push.sh`, never raw
  ...
  `scripts/mint_oi.sh` (§7, OI allocator row) — it creates a ref that must not exist, so its
...
|---|---|---|
| **A Postgres GUC with `context=postmaster` cannot be set via any SQL migration** — `ALTER DATABASE ... SET` fails outright (`55P02`, "cannot be changed without restarting the server"), it is not a retryable syntax issue | Before authorizing or drafting a migration that flips a server-level setting (anything under `cron.*`, `shared_buffers`, `max_connections`-class GUCs), check its context FIRST: `select context from pg_settings where name = '<setting>'`. Only `sighup`/`superuser`/`user` are reachable from a migration; `postmaster` needs a full server restart (Supabase dashboard/support), and `internal` is not settable at all. Describing a change as "just a GUC, fully reversible" before checking this is describing an assumption, not a fact — the two categories look identical in a migration file and only differ once you actually try to apply. | 2026-09-22 (disk-io-audit-cleanup work, diagnose e8b4a1) — `cron.log_run` was authorized as a simple reversible toggle; the restart requirement was only discovered on apply, after authorization, and the migration had to be withdrawn rather than forced through on a description that turned out to be wrong. |
| **Dropping an index because `idx_scan=0` checks only query-access usage — it says nothing about whether the same index backs a foreign key**, which Postgres also uses for referential-integrity checks on the referenced table's side regardless of whether any SELECT ever used it | Before dropping any `idx_scan=0` index, check `pg_constraint` for a foreign key on the same column(s): `select conname, pg_get_constraintdef(oid) from pg_constraint where conrelid = '<table>'::regclass and contype = 'f'`. If the index is the SOLE index covering an FK column, dropping it doesn't just remove dead weight — it removes referential-integrity-check coverage, which `get_advisors` will separately flag as a NEW `unindexed_foreign_keys` finding after the fact. Check every dropped index against this, not just the ones that "feel" FK-shaped. | 2026-09-22 (disk-io-audit-cleanup work, diagnose e8b4a1) — caught by a self-triggered B-pass review before commit, not before apply; `idx_nutrition_log_items_food_id` was dropped as "verified-dead" (true for query access) while being the sole FK-support index (a separate axis nobody checked), requiring a same-day follow-up migration (142) to restore it. |
