...
const regenerableIgnoredPaths = <String>[
  ...
  '.dart_tool/',
  'build/',
  // FVM's per-project SDK symlink/cache (.gitignore, added 2026-09-24), fully
  // reproduced from the committed `.fvmrc` by `fvm use <ver>` / `fvm install`.
  '.fvm/',
  // Deno lockfile — regenerated by any `deno test/check --node-modules-dir=none`
  // run (OI-153-era local Deno install). Without this entry, every worktree
  // that has ever run a Deno test is unretirable (ai-coach-ux-tool-integrity
  // batch, 2026-09-18 — the retirement dry-run KEEP'd on exactly this file).
  'deno.lock',
  '.flutter-plugins',
