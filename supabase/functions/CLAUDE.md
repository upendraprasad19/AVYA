...
| `gemini_failure_alert` (food-logging-observations, 2026-09-20) | `_shared/gemini_failure_alert.ts` `reportGeminiExhaustion(client, source, lastError, endpoint?)`, called from `ai-proxy/index.ts` at exactly 3 call sites — `food_text_analysis` (:443), `scan_meal` (:603), `cart_auditor` (:646) — on total Gemini attempt-list exhaustion. Inserts into `public.alerts`, reusing `trg_dispatch_critical_alert_notify` (migration 133) for the founder's existing Telegram alert channel — no new Telegram wiring. `source` stays constant across all 3 call sites so a 30-min dedup window spans them together; `endpoint` (the request type) is used only in `summary`/`context_json` so 3 otherwise-identical alerts stay distinguishable. Gated behind `DISABLE_GEMINI_FAILURE_ALERT` (§4.6 platform-tier kill-switch — the founder can silence it from the Edge Function secrets dashboard without a redeploy). **Deliberately NOT wired at ai-proxy's plain-chat call site (~:692) or the `geminiChatWithTools` tool-calling path (~:1009)** — Task 10's own plan scoped it to exactly the 3 nutrition AI endpoints; the gap is tracked as OI-226, not silently absent. | `alerts` table → Telegram, founder-only, zero client-facing effect either way. |
